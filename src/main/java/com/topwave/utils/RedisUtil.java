...
public class RedisUtil {
    ...
    private static final Logger log = Logger.getLogger(BuildingSynch.class);
    ...
    private RedisUtil(Prop prop) {
        this.prop = prop;
    }

    public static RedisUtil getInstance(Prop prop) {
        if (instance == null) {
            lock.lock();
            if (instance == null) {
                instance = new RedisUtil(prop);
            }
            lock.unlock();
        }
        return instance;
     ...
     * 初始化JedisPool
     */
    private void initJedisPool(Prop prop) {
//        properties = ConfigHelper.loadProperties("config.properties");
//        Prop prop = PropKit.use(SynchExecutor.getFile());
        String ip = prop.get("redis.ip");
        Integer port = Integer.valueOf(prop.get("redis.port"));
        String password = prop.get("redis.password");
        Integer database = Integer.valueOf(prop.get("redis.database"));
        Integer timeout = Integer.valueOf(prop.get("redis.timeout"));

        JedisPoolConfig config = new JedisPoolConfig();
        config.setMaxIdle(Integer.valueOf(prop.get("redis.pool.maxIdle")));
...
//        config.setTestOnReturn(Boolean.getBoolean(properties.getProperty("redis.pool.testOnReturn")));
        ...
        jedisPool = new JedisPool(
                config,ip, port,timeout,password,database
        );
    }
...
//===========================================================
    ...
    public class Keys {
         ...
         * @return
         */
        public void flushAll(String prefix) {
            Jedis jedis = getJedis();
            Set<String> keys = jedis.keys(prefix + "*");
            String[] keyArry = keys.toArray(new String[keys.size()]);
            if (keyArry.length > 0){
                jedis.del(keyArry);
            }
            closeJedis(jedis);
